...
import { fetchMySpheronStoreDatabases } from 'services/save-db-to-profile'
...
export default function MongoStepSelectCollections({
  ...
  const {
    ...
    setSpheronstoreDatabases,
  } = useDatabaseMigrationStore()

  const allChecked = checkedItems.every(Boolean)
  const isIndeterminate = checkedItems.some(Boolean) && !allChecked

  // get spheronstore dbs for the currently logged in user
  const getSpheronStoreDbs = async () => {
    let data = await fetchMySpheronStoreDatabases()
    setSpheronstoreDatabases(data)
  }
